...
class Sidebar {
  ...
  public createSidebarNode(lvls: number, currentLvl: number, completeLvls: number[]): HTMLElement {
    ...
    const addActive = (node: HTMLElement) => node.classList.add('bg-orange-400');
    const removeActive = (node: HTMLElement) => node.classList.remove('bg-orange-400');
    const addComplete = (node: HTMLElement) => node.classList.add('bg-green-400');
    const removeComplete = (node: HTMLElement) => node.classList.remove('bg-green-400');
    for (let i = 0; i < lvls + 1; i += 1) {
      const li: HTMLElement = node.createNode('li', 'flex'.split(' '));
      const a: HTMLElement = node.createNode('a', 'flex justify-center px-10 w-full py-1 bg-slate-500 hover:bg-slate-300 hover:text-slate-600 transition-colors'.split(' '), (i + 1).toString());
